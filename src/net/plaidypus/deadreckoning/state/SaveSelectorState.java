...
public class SaveSelectorState extends ExclusiveHudLayersState{
	...
	public SaveSelectorState(int stateID, ArrayList<HudElement> background) throws SlickException {
			...
			if(currentPressed.isPressed()){
				try {
					...
					if(this.focus>buttonIndex){
						saves[focus-buttonIndex-1].loadGame( GameplayElement.class.cast(HudLayersState.class.cast(DeadReckoningGame.instance.getState(DeadReckoningGame.GAMEPLAYSTATE)).getElement(0) ));
	...
	private static ArrayList<HudElement> makeElementsList() throws SlickException{
		...
		ArrayList<HudElement> buttons = new ArrayList<HudElement>(savesList.length+2);
		saves = new Save[savesList.length];
		
		buttons.add(new TextButton(10,30,HudElement.TOP_LEFT,new Color(30,50,70),new Color(40,60,80),new Color(60,80,100),"New Game",DeadReckoningGame.menuFont));
